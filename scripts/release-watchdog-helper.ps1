...
$tag = "watchdog-helper-v$Version"

Assert-TrackedTreeClean
Assert-UniqueTag $tag

Write-Host "Building WatchDog Helper before tagging $tag..."
Push-Location (Join-Path $repoRoot "WatchDogHelper")
try {
...
git tag -a $tag -m "WatchDog Helper v$Version"
git push origin $tag
