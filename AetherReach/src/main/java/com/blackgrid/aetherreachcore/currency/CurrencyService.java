...
import com.blackgrid.aetherreachcore.currency.storage.CurrencyStorage;
import com.blackgrid.aetherreachcore.currency.storage.SqliteCurrencyStorage;
...
public class CurrencyService {
    ...
    private static int getPositiveRankInteger(ServerPlayer player, String node) {
        if (ModList.get().isLoaded("ftbranks")) {
            try {
                ...
                AetherreachCore.LOGGER.warn("[Aetherreach] Failed to read FTB Ranks permission '{}': {}", node, e.getMessage());
            }
        }
    ...
    public static void reloadStorage() {
        storage.reload();
        AetherreachCore.LOGGER.info("[Aetherreach] Currency storage reloaded.");
    }
