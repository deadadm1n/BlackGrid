...
public final class BridgeService {
    ...
    public static synchronized void start(MinecraftServer server) {
        ...
        if (!Config.BRIDGE_ENABLED.get()) {
            AetherreachCore.LOGGER.info("[AetherReach Bridge] Bridge disabled in config.");
            return;
        }
        ...
        try {
            ...
            AetherreachCore.LOGGER.info("[AetherReach Bridge] Listening on http://{}:{}", host, port);
        } catch (IOException e) {
            AetherreachCore.LOGGER.error("[AetherReach Bridge] Failed to start bridge server", e);
        }
    }
    ...
    public static synchronized void stop() {
        if (httpServer != null) {
            httpServer.stop(1);
            httpServer = null;
            AetherreachCore.LOGGER.info("[AetherReach Bridge] Bridge stopped.");
        }
    ...
    private static void handleStatus(HttpExchange exchange) throws IOException {
        ...
        response.addProperty("mod", "aetherreachcore");
    ...
    private static JsonObject readJson(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            AetherreachCore.LOGGER.warn("[AetherReach Bridge] Invalid JSON request body: {}", e.getMessage());
            return null;
        }
    }
    ...
    private static boolean isAuthorized(JsonObject body) {
        ...
        if (configuredToken == null || configuredToken.isBlank() || "change-me".equals(configuredToken)) {
            AetherreachCore.LOGGER.warn("[AetherReach Bridge] bridgeToken is not configured securely.");
            return false;
        }
    ...
    private static MutableComponent aetherMessage(String text) {
        return Component.literal("[AetherReach] ")
                .withStyle(ChatFormatting.LIGHT_PURPLE)
                .append(Component.literal(text).withStyle(ChatFormatting.GRAY));
    }
