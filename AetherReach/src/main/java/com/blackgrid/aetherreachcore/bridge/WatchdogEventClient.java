...
public final class WatchdogEventClient {
    ...
    public static void sendChatEvent(String uuid, String player, String message) {
        ...
        if (token == null || token.isBlank() || "change-me".equals(token)) {
            AetherreachCore.LOGGER.warn("[AetherReach] Watchdog callback skipped because bridgeToken is not configured.");
            return;
        }
        ...
        CLIENT.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .exceptionally(error -> {
                    AetherreachCore.LOGGER.warn("[AetherReach] Failed to send chat event to Watchdog: {}", error.getMessage());
                    return null;
                });
    }
